...
// Données simulées pour SHOPBATI France (fallback)
const fallbackProducts: Product[] = [
  ...
  {
    $id: '2', 
    name: 'Briques Rouges Traditionnelles',
    description: 'Briques en terre cuite pour maçonnerie',
    price: 2.50,
...
const fallbackCategories: Category[] = [
  { $id: '1', name: 'Ciments & Mortiers', description: 'Ciments et liants hydrauliques', slug: 'ciments', status: 'active', sort_order: 1 },
  { $id: '2', name: 'Briques & Blocs', description: 'Briques et blocs de maçonnerie', slug: 'briques', status: 'active', sort_order: 2 },
  { $id: '3', name: 'Carrelage & Revêtements', description: 'Revêtements céramiques et sols', slug: 'carrelage', status: 'active', sort_order: 3 },
  { $id: '4', name: 'Métaux & Aciers', description: 'Fer et acier pour constructions', slug: 'metaux', status: 'active', sort_order: 4 },
  { $id: '5', name: 'Isolation Thermique', description: 'Matériaux isolants performants', slug: 'isolants', status: 'active', sort_order: 5 },
  { $id: '6', name: 'Peintures & Finitions', description: 'Peintures et vernis professionnels', slug: 'peintures', status: 'active', sort_order: 6 }
]
...
export default function HomePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch both products and categories in parallel for better performance
      const [productsResult, categoriesResult] = await Promise.allSettled([
        ...
        appwrite.getCategories([
          ...
          appwrite.Query.limit(12) // Increased limit to show more categories
        ])
      ])

      // Handle products result
      if (productsResult.status === 'fulfilled' && productsResult.value?.documents) {
        setFeaturedProducts(productsResult.value.documents as unknown as Product[])
      } else {
        console.warn('Failed to fetch products from database, using fallback data')
        setFeaturedProducts(fallbackProducts)
      }

      // Handle categories result
      if (categoriesResult.status === 'fulfilled' && categoriesResult.value?.documents) {
        const fetchedCategories = categoriesResult.value.documents as unknown as Category[]
      ...
      // Use fallback data if database fetch fails completely
      setFeaturedProducts(fallbackProducts)
      setCategories(fallbackCategories)
    } finally {
      setLoading(false)
    ...
    <div className="min-h-screen bg-neutral-50">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-32">
          <div className="text-center">
            ...
            <p className="text-xl lg:text-2xl text-white/90 mb-8 max-w-3xl mx-auto animate-fade-in-up">
              Votre plateforme de confiance pour matériaux de construction de qualité professionnelle en France
            </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              <h2 className="text-4xl lg:text-5xl font-bold text-neutral-900 mb-4">
                Pourquoi Choisir ShopBati France
              </h2>
              <p className="text-xl text-neutral-600 max-w-3xl mx-auto">
                Nous nous engageons à fournir les meilleurs matériaux avec un service exceptionnel partout en France
              </p>
      ...
      <section className="py-20 bg-neutral-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {categories.map((category) => {
                ...
                const getCategoryIcon = (name: string) => {
                  ...
                  if (lowerName.includes('ciment') || lowerName.includes('mortier') || lowerName.includes('macon')) return '🏗️'
                  if (lowerName.includes('brique') || lowerName.includes('bloc') || lowerName.includes('maçon')) return '🧱'
                  if (lowerName.includes('carrelage') || lowerName.includes('carreleur') || lowerName.includes('revêtement')) return '🔲'
                  if (lowerName.includes('métal') || lowerName.includes('acier') || lowerName.includes('menuisier') || lowerName.includes('serrurerie')) return '⚙️'
                  ...
                  if (lowerName.includes('outillage') || lowerName.includes('protection')) return '🔨'
                  return '🏠' // Default icon
                }

                // Map category names to background gradients
                const getCategoryGradient = (name: string) => {
                  const lowerName = name.toLowerCase()
                  if (lowerName.includes('ciment') || lowerName.includes('mortier') || lowerName.includes('macon')) return 'from-gray-400 to-gray-600'
                  if (lowerName.includes('brique') || lowerName.includes('bloc') || lowerName.includes('maçon')) return 'from-red-400 to-red-600'
                  ...
                  if (lowerName.includes('outillage') || lowerName.includes('protection')) return 'from-indigo-400 to-indigo-600'
                  return 'from-brand-400 to-brand-600' // Default gradient
                }
                ...
                return (
                  ...
                  >
                    <div className={`h-48 bg-gradient-to-br ${getCategoryGradient(category.name)} flex items-center justify-center relative overflow-hidden`}>
                      {category.image_url ? (
                        <img 
                    ...
                    <div className="p-6">
                      ...
                      <p className="text-neutral-600 mb-4 line-clamp-2">
                        {category.description || `Découvrez tous nos produits de ${category.name.toLowerCase()}`}
                      </p>
          ...
          {/* Show "View all categories" button */}
      ...
      <section className="py-20 bg-gradient-to-br from-brand-500 via-brand-600 to-brand-700 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative">
          ...
          <div className="grid md:grid-cols-3 gap-8 mt-16 pt-12 border-t border-white/20">
            ...
            <div className="text-center">
              <div className="text-3xl lg:text-4xl font-bold text-white mb-2">24h-48h</div>
              <div className="text-white/80">Livraison Rapide</div>
            </div>
          </div>
        </div>
      </section>
