  ...
  try {
    ...
    if (orderSaved.success) {
      ...
      try {
        ...
        // Generate PDF invoice with QR code and upload to Appwrite
        const invoiceResult = await generateInvoiceWithQRCode(invoiceData)
        ...
        if (error) {
          ...
          // Update order with invoice URL and status
          if (orderSaved.orderId) {
            try {
              ...
              console.log('✅ Order updated with invoice URL and status:', orderSaved.orderId)
            } catch (updateError) {
    ...
    console.error('Erreur API orders with QR:', error)
    return NextResponse.json({ 
      success: false, 
...
function generateEmailContent(orderData: OrderData, invoiceUrl: string): string {
    ...
    <body style="margin: 0; padding: 0; font-family: 'Inter', Arial, sans-serif; background-color: #f5f5f5;">
      <div style="max-width: 600px; margin: 0 auto; background-color: white; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
        ...
        <div style="padding: 30px;">
          ...
          <!-- QR Code Section -->
          <div style="background: linear-gradient(135deg, #212121 0%, #424242 100%); border: 3px solid #FFD700; border-radius: 12px; padding: 20px; margin-bottom: 30px; text-align: center;">
            <h3 style="margin: 0 0 16px 0; color: #FFD700; font-size: 18px;">
              📱 Accédez à votre facture en ligne
            </h3>
            <p style="color: white; margin: 0 0 16px 0; font-size: 14px;">
              Scannez le QR code sur votre facture ou cliquez sur le bouton ci-dessous :
            </p>
            <a href="${invoiceUrl}" style="display: inline-block; background: #FFD700; color: #212121; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px;">
          ...
          <div style="background: #fff9c4; border: 2px solid #FFD700; border-radius: 12px; padding: 20px; margin-bottom: 30px;">
            ...
            <div style="color: #212121; font-size: 14px; line-height: 1.8;">
              ...
              <p style="margin: 4px 0;"><strong>🧾 Facture :</strong> Voir pièce jointe PDF ou en ligne</p>
            </div>
          </div>
        ...
        <div style="background: #212121; color: white; padding: 25px; text-align: center;">
          ...
          <div style="border-top: 1px solid #424242; padding-top: 15px;">
            ...
            <p style="margin: 5px 0 0 0; font-size: 11px; opacity: 0.6;">
              6 Rue des Bateliers - Bureau 3, 92110 Clichy, France
            </p>
          </div>
        </div>
      </div>
